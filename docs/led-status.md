...
|---|---|---|
| Plugin zip | `/opt/octoprint/venv/lib/…/OctoPrint_LedStatus-1.0.0.dist-info` | generated by `make plugins` from `plugins/octoprint-led-status/` |
...
These steps are for manual updates or re-installs only. The `octoprint` profile build handles the initial deployment automatically, and the ZIP is a generated artifact rather than a tracked repository input.
